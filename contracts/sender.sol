...
contract priusDisperser {
    ...
    function disperseERC721s(
        address _token,
        address[] calldata _recipients,
        uint256[] calldata _tokenIds
    ) external {
        require(
            _recipients.length == _tokenIds.length, "mismatched recipients and tokenIds");

            for (uint256 i = 0; i < _recipients.length; i++) {
        assembly {
            
            let callData := mload(0x40)
            mstore(callData, 0x23b872dd)
            mstore(add(callData, 0x04), caller())
            
            // Calldata layout for dynamic array: [offset][length][data...]
            let recipientOffset := add(_recipients.offset, add(0x20, mul(i, 0x20)))
            calldatacopy(add(callData, 0x24), recipientOffset, 0x20)

            let tokenIdOffset := add(_tokenIds.offset, add(0x20, mul(i, 0x20)))
            calldatacopy(add(callData, 0x44), tokenIdOffset, 0x20)

            let callDataSize := 0x64

            //ccall to the token contract
            let success := call(
                gas(), // forward all gas
                _token, // ERC721 address
                0, // no ether to be sent
                callData, // pointer to call data
                callDataSize, // size of the call data
                0, // no need for return data
                0  // no need for return data
            )

                    if iszero(success) {
                        revert(0, 0) //revert if transfer failed
                }
            }
        }
    }
